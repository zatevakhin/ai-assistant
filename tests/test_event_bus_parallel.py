...
class TestEventBusParallelExecution:
    ...
    def test_parallel_execution_basic(self, event_bus):
        ...
        futures = []
        for i in range(3):
            _, future = event_bus.call_service_async("test_plugin", "long_task", 0.5, f"task{i}")
        ...
        assert elapsed_time < 1.0, f"Execution took {elapsed_time:.2f}s, suggesting sequential execution"
    ...
    def test_tasks_different_durations(self, event_bus):
        ...
        start_time = time.time()
        _, long_future = event_bus.call_service_async("test_plugin", "long_task", 1.0, "long")

        # Small delay to ensure the first task starts first
        time.sleep(0.1)

        # Start a shorter task (0.3s) after the long one has started
        _, short_future = event_bus.call_service_async("test_plugin", "long_task", 0.3, "short")
        ...
        assert elapsed_time > 1.0, f"Long task completed too quickly: {elapsed_time:.2f}s"
    ...
    def test_cancel_task(self, event_bus):
        ...
        request_id, future = event_bus.call_service_async("test_plugin", "long_task", 10.0, "very_long")
